...
function MobileTaskCard({
  ...
  return (
    <div
      onClick={() => onEdit(task)}
      className={cn(
        "relative rounded-2xl p-3.5 bg-white/[0.02] border border-white/[0.06] hover:border-white/10 active:scale-[0.99] transition-all cursor-pointer shadow-sm",
    ...
    >
      <div className="flex items-start gap-3">
        ...
        <button
          ...
          className="mt-0.5 shrink-0 text-muted-foreground hover:text-emerald-400 transition-colors p-0.5"
        >
        ...
        <div className="flex-1 min-w-0 pr-1">
          <div className="flex items-start gap-1.5">
            {task.pinned && <Pin className="w-3 h-3 text-[#FFC107] shrink-0 mt-0.5" />}
          ...
          <div className="flex items-center gap-2 flex-wrap mt-2">
              ...
              <span
                className={cn(
                  ...
                  isOverdue
                    ...
                    : "bg-white/[0.03] text-muted-foreground border-white/05"
                )}
              >
            ...
            {task.status === "doing" && (
              <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded bg-[#FFC107]/10 text-[#FFC107] border border-[#FFC107]/20">
                In Progress
              </span>
            )}
          </div>
              ...
              {menuOpen && (
                ...
                >
                  <button
                    ...
                    className="w-full flex items-center gap-2 px-3 py-2 text-xs text-foreground/80 hover:bg-white/5"
                  >
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="h-full flex flex-col relative pb-20 md:pb-6">
      ...
      <div className="flex-1 px-4 py-2">
        {loading ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-center p-8 mt-6">
            <div className="w-12 h-12 rounded-full bg-white/[0.02] border border-white/10 flex items-center justify-center mb-3 text-[#FFC107]">
              <CheckSquare className="w-6 h-6" strokeWidth={1.5} />
            </div>
            <h3 className="font-bold text-sm text-foreground">No tasks found</h3>
            <p className="text-xs text-muted-foreground mt-1 max-w-[200px]">
              You're clear for now or no tasks match this filter.
            </p>
          </div>
        ) : view === "kanban" ? (
